...
class TextEditor:
    ...
    def get_key(self):
        if sys.platform == 'win32':
            ...
            # ESC sequence with extended prefix
            if o == 27:
            ...
            # Extended keys (arrows, Delete, etc) - prefix 224
            if o == 224:
                c2 = msvcrt.getwch()
                if not c2: return None
                # Handle both ASCII code and Unicode character for extended keys
                c2o = ord(c2)
                ...
                if not name and c2 == 'S': name = 'KEY_DELETE'  # Unicode 'S' for Delete
                if not name and c2 == 'G': name = 'KEY_HOME'  # Unicode 'G' for Home
                if not name and c2 == 'O': name = 'KEY_END'   # Unicode 'O' for End
