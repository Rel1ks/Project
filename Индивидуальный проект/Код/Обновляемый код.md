#Yougile #YouGile #код #Проект
```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Universal Launcher - Консольное приложение для Windows и Linux
Полная перерисовка экрана после каждого действия
"""

import json
import os
import platform
import subprocess
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Dict, List, Optional, Tuple


@dataclass
class AppInfo:
    name: str
    command: str
    category: str
    description: str = ""
    icon: str = "📦"


class UniversalLauncher:
    def __init__(self):
        self.os_type = platform.system().lower()
        self.is_windows = self.os_type == "windows"
        self.is_linux = self.os_type == "linux"

        if not (self.is_windows or self.is_linux):
            print("❌ Поддерживаются только Windows и Linux")
            sys.exit(1)

        self.apps: Dict[str, AppInfo] = {}
        self.favorites: List[str] = []
        self.config_dir = Path.home() / ".universal_launcher"
        self.config_file = self.config_dir / "config.json"
        self.last_message = ""
        self.last_message_type = ""  # 'success', 'error', 'info'

        self._ensure_config()
        self._detect_apps()
        self._load_favorites()

    def _clear_and_redraw(self):
        """Очистка экрана и полная перерисовка интерфейса"""
        # Очистка экрана
        if self.is_windows:
            os.system("cls")
        else:
            os.system("clear")

        # Перерисовка всего интерфейса
        self._draw_header()
        self._draw_apps()
        self._draw_help()
        self._draw_message()

    def _draw_header(self):
        """Рисуем заголовок"""
        os_icon = "🪟" if self.is_windows else "🐧"
        os_name = "Windows" if self.is_windows else "Linux"

        print(f"╔{'═' * 58}╗")
        print(f"║{'🚀 UNIVERSAL LAUNCHER'.center(56)}║")
        print(f"║{f'{os_icon} {os_name}'.center(56)}║")
        print(f"╠{'═' * 58}╣")
        print(
            f"║ Приложений: {len(self.apps):<3} │ Избранное: {len(self.favorites):<3}            ║"
        )
        print(f"╚{'═' * 58}╝")

    def _draw_apps(self):
        """Рисуем список приложений"""
        if not self.apps:
            print("\n  ⚠️  Не найдено приложений")
            return

        # Группировка по категориям
        categories: Dict[str, List[Tuple[str, AppInfo]]] = {}
        for key, app in self.apps.items():
            if app.category not in categories:
                categories[app.category] = []
            categories[app.category].append((key, app))

        print("\n" + "─" * 60)

        self.index_map: Dict[int, str] = {}
        idx = 1

        for cat in sorted(categories.keys()):
            apps = sorted(categories[cat], key=lambda x: x[1].name)
            print(f"\n  📂 {cat}")
            print("  " + "─" * 40)

            for key, app in apps:
                self.index_map[idx] = key
                fav = "⭐" if key in self.favorites else "  "
                # Форматируем вывод в одну строку
                line = f"  {idx:2}.{fav} {app.icon} {app.name}"
                print(line[:58])
                idx += 1

        print("\n" + "─" * 60)

    def _draw_help(self):
        """Рисуем справку"""
        print("""
  ┌─────────────────────────────────────────────────────────┐
  │  💡 КОМАНДЫ: [число] запуск │ [имя] запуск │ [URL]     │
  │     fav [число] - в избранное  │  favs - показать       │
  │     search [текст] - поиск  │  refresh - обновить       │
  │     help - справка  │  quit/q - выход                    │
  └─────────────────────────────────────────────────────────┘""")

    def _draw_message(self):
        """Рисуем сообщение (если есть)"""
        if self.last_message:
            icon = {"success": "✅", "error": "❌", "info": "ℹ️ ", "warning": "⚠️ "}.get(
                self.last_message_type, "•"
            )

            print(f"\n  {icon} {self.last_message}")
            print()  # Пустая строка для отступа

    def _set_message(self, text: str, msg_type: str = "info"):
        """Установить сообщение для отображения"""
        self.last_message = text
        self.last_message_type = msg_type

    def _clear_message(self):
        """Очистить сообщение"""
        self.last_message = ""
        self.last_message_type = ""

    def _ensure_config(self):
        """Создание директории конфигурации"""
        self.config_dir.mkdir(exist_ok=True)

    def _save_favorites(self):
        """Сохранение избранного"""
        try:
            with open(self.config_file, "w", encoding="utf-8") as f:
                json.dump({"favorites": self.favorites}, f)
        except:
            pass

    def _load_favorites(self):
        """Загрузка избранного"""
        try:
            if self.config_file.exists():
                with open(self.config_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    self.favorites = data.get("favorites", [])
        except:
            pass

    def _run_command(self, command: str, shell: bool = False) -> bool:
        """Универсальный запуск команды"""
        try:
            if self.is_windows:
                if shell or any(
                    cmd in command.lower()
                    for cmd in [
                        "start ",
                        "explorer",
                        "notepad",
                        "calc",
                        "cmd",
                        "powershell",
                    ]
                ):
                    subprocess.Popen(
                        command,
                        shell=True,
                        creationflags=subprocess.CREATE_NEW_PROCESS_GROUP,
                    )
                else:
                    subprocess.Popen(
                        command,
                        shell=False,
                        creationflags=subprocess.CREATE_NEW_PROCESS_GROUP,
                    )
            else:
                subprocess.Popen(
                    command,
                    shell=True,
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                    start_new_session=True,
                )
            return True
        except Exception as e:
            self._set_message(f"Ошибка запуска: {e}", "error")
            return False

    def _which(self, program: str) -> Optional[str]:
        """Поиск программы в PATH"""
        try:
            if self.is_windows:
                result = subprocess.run(
                    ["where", program], capture_output=True, text=True
                )
            else:
                result = subprocess.run(
                    ["which", program], capture_output=True, text=True
                )

            if result.returncode == 0:
                return result.stdout.strip().split("\n")[0]
        except:
            pass
        return None

    def _is_url(self, text: str) -> bool:
        """Проверка URL"""
        text = text.strip()
        return text.startswith(("http://", "https://", "www.")) or (
            "." in text and " " not in text and len(text) > 4
        )

    def _open_url(self, url: str) -> bool:
        """Открытие URL"""
        url = url.strip()
        if url.startswith("www."):
            url = "https://" + url

        try:
            if self.is_windows:
                os.system(f'start "" "{url}"')
            else:
                try:
                    subprocess.run(["xdg-open", url], check=True)
                except:
                    import webbrowser

                    webbrowser.open(url)
            self._set_message(f"Открыто: {url[:50]}", "success")
            return True
        except Exception as e:
            self._set_message(f"Не удалось открыть: {e}", "error")
            return False

    def _scan_windows(self):
        """Сканирование Windows приложений"""
        system_commands = {
            "notepad": ("Блокнот", "Системные", "Текстовый редактор", "📝"),
            "calc": ("Калькулятор", "Системные", "Калькулятор", "🧮"),
            "explorer": ("Проводник", "Системные", "Файловый менеджер", "📁"),
            "cmd": ("Командная строка", "Системные", "CMD", "💻"),
            "powershell": ("PowerShell", "Системные", "PowerShell", "⚡"),
            "mspaint": ("Paint", "Системные", "Графический редактор", "🎨"),
            "taskmgr": ("Диспетчер задач", "Системные", "Task Manager", "📊"),
            "control": ("Панель управления", "Системные", "Control Panel", "⚙️"),
        }

        for cmd, (name, cat, desc, icon) in system_commands.items():
            self.apps[cmd] = AppInfo(name, cmd, cat, desc, icon)

        program_files = [
            os.environ.get("ProgramFiles", "C:\\Program Files"),
            os.environ.get("ProgramFiles(x86)", "C:\\Program Files (x86)"),
            os.path.expanduser("~\\AppData\\Local"),
            os.path.expanduser("~\\AppData\\Roaming"),
        ]

        known_apps = {
            "chrome": (
                "Google\\Chrome\\Application\\chrome.exe",
                "Браузеры",
                "Google Chrome",
                "🌐",
            ),
            "firefox": (
                "Mozilla Firefox\\firefox.exe",
                "Браузеры",
                "Mozilla Firefox",
                "🦊",
            ),
            "edge": (
                "Microsoft\\Edge\\Application\\msedge.exe",
                "Браузеры",
                "Microsoft Edge",
                "🌊",
            ),
            "opera": ("Opera\\launcher.exe", "Браузеры", "Opera", "🔴"),
            "code": ("Microsoft VS Code\\Code.exe", "Разработка", "VS Code", "💻"),
            "sublime": (
                "Sublime Text\\sublime_text.exe",
                "Разработка",
                "Sublime Text",
                "📝",
            ),
            "notepad++": ("Notepad++\\notepad++.exe", "Разработка", "Notepad++", "📝"),
            "pycharm": (
                "JetBrains\\PyCharm\\bin\\pycharm64.exe",
                "Разработка",
                "PyCharm",
                "🐍",
            ),
            "discord": ("Discord\\Discord.exe", "Коммуникации", "Discord", "💬"),
            "telegram": (
                "Telegram Desktop\\Telegram.exe",
                "Коммуникации",
                "Telegram",
                "✈️",
            ),
            "steam": ("Steam\\steam.exe", "Игры", "Steam", "🎮"),
            "spotify": ("Spotify\\Spotify.exe", "Медиа", "Spotify", "🎵"),
            "vlc": ("VideoLAN\\VLC\\vlc.exe", "Медиа", "VLC", "🎬"),
            "obs": ("obs-studio\\bin\\64bit\\obs64.exe", "Стриминг", "OBS", "📺"),
            "gimp": ("GIMP 2\\bin\\gimp-2.10.exe", "Графика", "GIMP", "🎨"),
        }

        for key, (rel_path, cat, name, icon) in known_apps.items():
            for base in program_files:
                full_path = os.path.join(base, rel_path)
                if os.path.exists(full_path):
                    self.apps[key] = AppInfo(name, full_path, cat, "", icon)
                    break

        # PowerShell Start Menu
        try:
            ps_cmd = "Get-StartApps | Select-Object -First 30 | ConvertTo-Json"
            result = subprocess.run(
                ["powershell", "-Command", ps_cmd], capture_output=True, text=True
            )
            if result.returncode == 0:
                import json

                apps = json.loads(result.stdout)
                if not isinstance(apps, list):
                    apps = [apps]
                for app in apps:
                    if "Name" in app and "AppID" in app:
                        name = app["Name"]
                        key = name.lower().replace(" ", "_").replace("-", "_")[:20]
                        if key not in self.apps and len(name) < 40:
                            self.apps[key] = AppInfo(
                                name,
                                f'start "" "{app["AppID"]}"',
                                "Меню Пуск",
                                "",
                                "📌",
                            )
        except:
            pass

    def _scan_linux(self):
        """Сканирование Linux приложений"""
        desktop_paths = [
            "/usr/share/applications",
            os.path.expanduser("~/.local/share/applications"),
            "/usr/local/share/applications",
        ]

        for d_path in desktop_paths:
            if os.path.exists(d_path):
                for desktop_file in Path(d_path).glob("*.desktop"):
                    self._parse_desktop_file(desktop_file)

        cli_apps = [
            ("firefox", "Браузеры", "Firefox", "🦊"),
            ("chromium", "Браузеры", "Chromium", "🌐"),
            ("google-chrome", "Браузеры", "Chrome", "🌐"),
            ("brave", "Браузеры", "Brave", "🦁"),
            ("code", "Разработка", "VS Code", "💻"),
            ("subl", "Разработка", "Sublime", "📝"),
            ("vim", "Разработка", "Vim", "📝"),
            ("nano", "Разработка", "Nano", "📝"),
            ("gedit", "Разработка", "Gedit", "📝"),
            ("gimp", "Графика", "GIMP", "🎨"),
            ("inkscape", "Графика", "Inkscape", "🎨"),
            ("vlc", "Медиа", "VLC", "🎬"),
            ("spotify", "Медиа", "Spotify", "🎵"),
            ("discord", "Коммуникации", "Discord", "💬"),
            ("telegram-desktop", "Коммуникации", "Telegram", "✈️"),
            ("steam", "Игры", "Steam", "🎮"),
            ("obs", "Стриминг", "OBS", "📺"),
            ("blender", "3D", "Blender", "🎲"),
            ("terminator", "Терминал", "Terminator", "💻"),
            ("konsole", "Терминал", "Konsole", "💻"),
            ("gnome-terminal", "Терминал", "GNOME Terminal", "💻"),
        ]

        for cmd, cat, name, icon in cli_apps:
            if self._which(cmd) and cmd not in self.apps:
                self.apps[cmd] = AppInfo(name, cmd, cat, "", icon)

        # Flatpak
        if self._which("flatpak"):
            try:
                result = subprocess.run(
                    ["flatpak", "list", "--app", "--columns=application,name"],
                    capture_output=True,
                    text=True,
                )
                if result.returncode == 0:
                    for line in result.stdout.strip().split("\n")[1:]:
                        parts = line.split("\t")
                        if len(parts) >= 2:
                            app_id = parts[0].strip()
                            name = parts[1].strip()
                            key = name.lower().replace(" ", "_")[:20]
                            if key not in self.apps:
                                self.apps[key] = AppInfo(
                                    name, f"flatpak run {app_id}", "Flatpak", "", "📦"
                                )
            except:
                pass

    def _parse_desktop_file(self, path: Path):
        """Парсинг .desktop файла"""
        try:
            name = exec_cmd = categories = None

            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                in_entry = False
                for line in f:
                    line = line.strip()
                    if line == "[Desktop Entry]":
                        in_entry = True
                    elif line.startswith("[") and in_entry:
                        break
                    elif in_entry:
                        if line.startswith("Name=") and not name:
                            name = line[5:]
                        elif line.startswith("Exec=") and not exec_cmd:
                            exec_cmd = line[5:]
                            for p in ["%f", "%F", "%u", "%U", "%c", "%k", "%i"]:
                                exec_cmd = exec_cmd.replace(p, "")
                            exec_cmd = exec_cmd.strip()
                        elif line.startswith("Categories="):
                            categories = line[11:].split(";")[0]
                        elif "NoDisplay=true" in line or "Hidden=true" in line:
                            return

            if name and exec_cmd:
                cmd_parts = exec_cmd.split()
                if cmd_parts:
                    cmd_name = cmd_parts[0]
                    if cmd_name.startswith("/"):
                        if not os.path.exists(cmd_name):
                            return
                    elif not self._which(cmd_name):
                        return

                key = name.lower().replace(" ", "_").replace("-", "_")[:25]
                cat = categories if categories else "Приложения"
                if key not in self.apps:
                    self.apps[key] = AppInfo(name, exec_cmd, cat)
        except:
            pass

    def _detect_apps(self):
        """Основной метод сканирования"""
        self.apps.clear()
        if self.is_windows:
            self._scan_windows()
        else:
            self._scan_linux()
        self.apps = dict(sorted(self.apps.items(), key=lambda x: x[1].name))

    def search_app(self, query: str) -> List[Tuple[str, AppInfo]]:
        """Поиск приложений"""
        query = query.lower()
        return [
            (k, v)
            for k, v in self.apps.items()
            if query in k or query in v.name.lower()
        ]

    def run(self):
        """Главный цикл с полной перерисовкой"""
        # Первоначальная отрисовка
        self._clear_and_redraw()

        while True:
            try:
                # Позиционируем курсор внизу для ввода
                print("\n  ➤ ", end="", flush=True)
                user_input = input().strip()

                if not user_input:
                    self._clear_message()
                    self._clear_and_redraw()
                    continue

                parts = user_input.split(maxsplit=1)
                cmd = parts[0].lower()
                arg = parts[1] if len(parts) > 1 else ""

                # Обработка команд

                if cmd in ["quit", "q", "exit"]:
                    self._clear_and_redraw()
                    self._set_message("До свидания!", "info")
                    self._draw_message()
                    break

                elif cmd in ["help", "?", "h"]:
                    self._set_message("Справка отображена выше", "info")

                elif cmd == "refresh":
                    self._set_message("Обновление списка...", "info")
                    self._clear_and_redraw()
                    self._detect_apps()
                    self._set_message(f"Найдено {len(self.apps)} приложений", "success")

                elif cmd in ["list", "ls", "l"]:
                    self._clear_message()

                elif cmd == "search":
                    if arg:
                        results = self.search_app(arg)
                        if results:
                            self._set_message(
                                f"Найдено: {', '.join([a.name for _, a in results[:5]])}",
                                "success",
                            )
                        else:
                            self._set_message("Ничего не найдено", "warning")
                    else:
                        self._set_message("Введите текст для поиска", "warning")

                elif cmd == "fav":
                    if arg.isdigit():
                        num = int(arg)
                        if num in self.index_map:
                            key = self.index_map[num]
                            if key in self.favorites:
                                self.favorites.remove(key)
                                self._set_message(
                                    f"Удалено из избранного: {self.apps[key].name}",
                                    "success",
                                )
                            else:
                                self.favorites.append(key)
                                self._set_message(
                                    f"Добавлено в избранное: {self.apps[key].name}",
                                    "success",
                                )
                            self._save_favorites()
                        else:
                            self._set_message("Неверный номер", "error")
                    else:
                        self._set_message("Использование: fav [номер]", "warning")

                elif cmd in ["favs", "favorites"]:
                    if self.favorites:
                        names = [
                            self.apps[k].name for k in self.favorites if k in self.apps
                        ]
                        self._set_message(f"Избранное: {', '.join(names[:5])}", "info")
                    else:
                        self._set_message("Избранное пусто", "warning")

                elif cmd.isdigit():
                    num = int(cmd)
                    if num in self.index_map:
                        key = self.index_map[num]
                        app = self.apps[key]
                        self._set_message(f"Запуск: {app.name}...", "success")
                        self._clear_and_redraw()  # Перерисовка перед запуском
                        self._run_command(app.command)
                    else:
                        self._set_message("Неверный номер", "error")

                elif self._is_url(user_input):
                    self._open_url(user_input)

                else:
                    # Поиск по имени
                    if cmd in self.apps:
                        app = self.apps[cmd]
                        self._set_message(f"Запуск: {app.name}...", "success")
                        self._clear_and_redraw()
                        self._run_command(app.command)
                    else:
                        matches = self.search_app(cmd)
                        if len(matches) == 1:
                            key, app = matches[0]
                            self._set_message(f"Запуск: {app.name}...", "success")
                            self._clear_and_redraw()
                            self._run_command(app.command)
                        elif len(matches) > 1:
                            names = [a.name for _, a in matches[:5]]
                            self._set_message(
                                f"Найдено несколько: {', '.join(names)}", "warning"
                            )
                        else:
                            self._set_message(f"Команда '{cmd}' не найдена", "error")

                # Перерисовка после каждого действия
                self._clear_and_redraw()

            except KeyboardInterrupt:
                self._clear_and_redraw()
                self._set_message("До свидания!", "info")
                self._draw_message()
                break
            except Exception as e:
                self._set_message(f"Ошибка: {e}", "error")
                self._clear_and_redraw()


def main():
    if sys.version_info < (3, 7):
        print("❌ Требуется Python 3.7+")
        sys.exit(1)

    try:
        app = UniversalLauncher()
        app.run()
    except Exception as e:
        print(f"Критическая ошибка: {e}")
        sys.exit(1)


if __name__ == "__main__":
    main()

```